...
def to_dict(accession: str, target: str, region: str, root: str):
    df = pd.read_csv(
        "{root}/{region}_parsed/{accession}.bed.gz".format(
            root=root,
            region=region,
            accession=accession
...
def concatenate(root: str, cell_lines: List[str], workers: int):
    ...
    for region in tqdm(("promoters", "enhancers"), leave=False, desc="Concatenating regions"):
        ...
        for cell_line, group in tqdm(filtered.groupby("cell_line"), leave=False, desc="Concatenating cell lines"):
            path = f"{root}_{region}_{cell_line}.csv.gz"
